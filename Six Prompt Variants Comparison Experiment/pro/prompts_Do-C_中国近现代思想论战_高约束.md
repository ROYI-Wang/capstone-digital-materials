# Do-C — Modern Chinese Debate Skill（中国近现代思想论战，高约束）

> 用途：通用方法论 + 中国近现代思想论战文体/语境适配，不含科玄论战专名  
> 领域范围：中国近现代思想论战（不限具体议题）  
> 约束强度：高（完整 schema，严格类型/关系枚举，16 条原则，自检清单）
...
1. **metadata** — 文本元数据
2. **entities** — 实体：文本中明确出现的人物、知识阵营/思想群体、核心争论概念、主义/学说/理论、方法、著作/报刊、历史事件等。不要预设任何具体人名、阵营名或概念名。
...
- 辩论者常分属不同知识阵营或思想流派
- 文本可能含有典故化用、形象化批评、贬称标签等具有时代特色的修辞

只有当前文本中明确出现的内容才可以抽取。以上提示仅用于帮助理解文本语境。
...
### 6. 群体不能误标为人物
论战中常出现群体性对象（如"科学家""唯物论者""青年学生""知识分子"等集合性称谓），应标为"阵营/群体"，而非"人物"。
...
### 11. 保留问题节点
如果文本涉及争论焦点（如某一核心概念能否统摄另一领域、两种立场之间的界限是否存在等），应标为"问题"类型实体。
...
4. 对重要思想人物（中西传统思想家与学者）的提及必须标注引用 function（援引权威支持 / 援引权威反驳 / 作为批评对象 / 中性提及），不可留空。
...
| 对比维度 | Do-C（本版） | Sp-C（原版） | Ge-C（预定） |
|---|---|---|---|
| 领域范围 | 中国近现代思想论战 | 科玄论战 | 中文论战通用 |
| 系统角色 | 提到中西/古今/科学人文等主题 | 提到"科玄论战" | 不限定具体论战主题 |
| 实体举例 | 无具体人名/概念名 | 丁文江、张君劢、科学、人生观… | 无 |
| 文体提示 | 有（中西之辨、援引权威等） | 有（科玄论战具体对象） | 无 |
| 修辞举例 | 通用（贬称、标签化、典故化用） | 含科玄专属词（玄学鬼、打鬼…） | 通用 |
| 16 条原则 | ✅ 全部保留 | ✅ | ✅ |
| 类型枚举 | ✅ 严格 | ✅ | ✅ |
| 自检清单 | ✅ | ✅ | ✅ |
| 与 D 共用的内容 | 全部 schema、relation、claim_type、polarity、JSON 格式、自检清单 | — | — |
