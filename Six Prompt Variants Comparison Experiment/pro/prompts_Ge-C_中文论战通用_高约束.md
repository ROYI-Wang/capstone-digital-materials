# Ge-C — Schema-constrained Chinese Debate Skill（中文论战通用，高约束）

> 用途：测试完整方法论在不接触任何领域知识时的表现——即"纯方法"的上限  
> 领域范围：不限时代与题材的中文论战文本（不含任何领域提示）  
> 约束强度：高（16 条原则，严格类型/关系枚举，自检清单）
...
1. **metadata** — 文本元数据
2. **entities** — 实体：文本中明确出现的人物、群体/阵营、概念、主义/学说/理论、方法、著作/文章、历史事件等。不要预设任何具体人名、阵营名或概念名。
...
### 6. 群体不能误标为人物
论战中常出现群体性对象（集合性称谓、思想流派、职业群体等），应标为"阵营/群体"，而非"人物"。
...
### 10. 保留定义与概念边界
如果文本出现"X 是……""所谓 X……""X 可分为……""X 不同于 Y……""X 不是……""X 包括……"，应抽取 definitions。

### 11. 保留问题节点
如果文本涉及争论焦点（论战中各方争执的核心问题），应标为"问题"类型实体。

### 12. 保留引用功能
如果作者引用第三方人物、著作或观点，必须说明引用 function：援引权威支持、援引权威反驳、作为历史证据、作为思想来源、作为批评对象、中性提及。

### 13. 保留修辞功能
如果文本使用明显修辞（如贬称、标签化、比喻、类比、讽刺、反语、夸张、归谬、俗语或形象化表达），应抽取 rhetorical_devices，并说明其论证功能。
...
当 relation 为批评/反驳/质疑/回应/重新定义/区分/混淆 时，
...
- 部分批评：肯定中有批评
- 混合评价：既肯定又批评，态度复杂
- 同阵营内部批评：批评者与被批评者属相近阵营，但在具体问题上分歧
- 一般批评：无法归入以上类别
- 不适用：该关系不需要子类型
...
4. 对重要思想人物的提及必须标注引用 function（援引权威支持 / 援引权威反驳 / 作为批评对象 / 中性提及），不可留空。
