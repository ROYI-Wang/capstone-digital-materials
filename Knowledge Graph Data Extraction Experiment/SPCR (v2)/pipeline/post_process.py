...
def _fix_influence_direction(combined: dict) -> dict:
    ...
    for r in combined.get("relations", []):
        ...
        r["head"], r["tail"] = r["tail"], r["head"]
        r["head_type"], r["tail_type"] = r["tail_type"], r["head_type"]
        if "stance_score" in r:
            r["stance_score"] = -r.get("stance_score", 0.0)
...
# --- H10. 补全 relation 必填字段默认值 ---

def _fill_relation_defaults(combined: dict) -> dict:
    """确保每条 relation 都有 relation_subtype / stance_score / intensity"""
    filled = 0
    for r in combined.get("relations", []):
        if "relation_subtype" not in r:
            r["relation_subtype"] = ""
            filled += 1
        if "stance_score" not in r:
            polarity = r.get("polarity", "").lower()
            if polarity == "positive":
                ...
                r["stance_score"] = 0.0
            filled += 1
        if "intensity" not in r:
            r["intensity"] = 0.5
            filled += 1
    if filled:
        print(f"  H10. relation 补全 {filled} 个缺失字段")
    return combined
...
def final_polish(combined: dict, segments: list) -> dict:
    ...
    combined = _fill_relation_defaults(combined)
