...
from config import (
    ...
    PROJECT_NAME, SCHEMA_VERSION, PROMPT_VERSION,
...
def _try_float(x: Any, default: float = 0.0, low: float = -1.0, high: float = 1.0) -> float:
    try:
        v = float(x)
        if v < low:
            return low
...
注意：polarity 字段保留用于兼容，但 stance_score 和 intensity **必须填写**（不可省略字段），下游分析优先使用 stance_score。
...
> **⚠️ 必填警告**：`relations` 中的 `relation_subtype`、`stance_score`、`intensity` 三个字段**必须**出现在每一条 relation 的 JSON 对象中，**严禁省略**。即使无信息，也要填写空字符串（`""`）或零值（`0.0`）。
...
### 立场评分检查（新增）
- 每条 relation 的 relation_subtype 是否已填写（至少填 `""`）？
...
def normalize_extract_schema(obj: Dict[str, Any], meta: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for i, x in enumerate(obj.get("relations") or [], 1):
        ...
        out["relations"].append({
            ...
            "relation": relation, "relation_subtype": _s(x.get("relation_subtype") or ""),
            "tail": tail, "tail_type": coerce_entity_type(x.get("tail_type") or ""),
            "polarity": coerce_polarity(x.get("polarity")),
            "stance_score": _try_float(x.get("stance_score"), 0.0, -1.0, 1.0),
            "intensity": _try_float(x.get("intensity"), 0.5, 0.0, 1.0),
            "evidence": evidence, "confidence": _conf(x.get("confidence")),
        })
