...
foreach ($f in $files) {
    ...
    try {
        ...
        $report += @{text=$txt; entities=0; claims=0; relations=0; defs=0; cits=0; rhet=0}
        continue
    }
    ...
    $etypeHash = @{}
    $engTypes = @(); $emptyName = 0; $emptyType = 0
    ...
    foreach ($r in $relations) {
        ...
        $rtypeHash[$rt]++
        if ($rt -in $deprecated) { $depFound += $rt }
